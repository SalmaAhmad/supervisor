...
void CMNodeHelloCM::odometryFillMsg(nav_msgs::msg::Odometry& msg){
  msg.header.frame_id = "map";
  msg.header.stamp = rclcpp::Clock().now();
  msg.child_frame_id = "FR1A";

  msg.pose.pose.position.x = Car.Fr1.t_0[0];
  msg.pose.pose.position.y = Car.Fr1.t_0[1];
  msg.pose.pose.position.z = Car.Fr1.t_0[2];

  float angle_x = Car.Fr1.r_zyx[0];
  float angle_y = Car.Fr1.r_zyx[1];
  float angle_z = Car.Fr1.r_zyx[2];
  ...
  msg.twist.twist.linear.x = Car.Fr1.v_0[0];
  msg.twist.twist.linear.y = Car.Fr1.v_0[1];
  msg.twist.twist.linear.z = Car.Fr1.v_0[2];

  msg.twist.twist.angular.x = Car.RollVel;
  msg.twist.twist.angular.y = Car.PitchVel;
  msg.twist.twist.angular.z = Car.YawRate;
  ...
  transform.transform.translation.x = Car.Fr1.t_0[0];
  transform.transform.translation.y = Car.Fr1.t_0[1];
  transform.transform.translation.z = Car.Fr1.t_0[2];
  transform.transform.rotation = tf2::toMsg(q);

  CMNode.TF.br->sendTransform(transform);
...
void CMNodeHelloCM::FullLandmarkListFillMsg(asurt_msgs::msg::LandmarkArray& msg) {

  if (CMNode.Sensor.Object.Active) {   

    asurt_msgs::msg::Landmark markers;
    ...
    for (int j = 0; j < ObjectSensor[0].nObsvObjects; j++) {
      ...
      markers.identifier = pOSO->ObjId;

    
      markers.type=4;

      if (pOSO->dtct || pOSO->InLane){
            if(pOSO->l!=0.228&&pOSO->w!=0.228&&pOSO->h!=0.325){markers.type=3;}

        std::string objInfo = pObj->Cfg.Info; 
        if ((objInfo.find("Y") != std::string::npos)||(objInfo.find("y") != std::string::npos)) {
          /* yellow */
          markers.type=1;
      ...
      markers.position.z = pObj->t_0[2];
      markers.position.x = pObj->t_0[0];
      markers.position.y = pObj->t_0[1];
      // markers.pose.position.x = obj_center.getX();
      // markers.pose.position.y = obj_center.getY();
      // markers.pose.position.z = obj_center.getZ();

      msg.landmarks.push_back(markers);
    } // for (int j = 0; j < ObjectSensor[0].nObsvObjects; j++)
  } // if (CMNode.Sensor.Object.Active)
}

void CMNodeHelloCM::landmarklistFillMsg(asurt_msgs::msg::LandmarkArray& msg) {
...
void CMNodeHelloCM::objectlistFillMsg(visualization_msgs::msg::MarkerArray& msg) {
  ...
  if (CMNode.Sensor.Object.Active) {   
    ...
    for (int j = 0; j < ObjectSensor[0].nObsvObjects; j++) {
      ...
      markers.ns = pObj->Cfg.Info;
      markers.type = visualization_msgs::msg::Marker::CYLINDER;
      markers.action = visualization_msgs::msg::Marker::ADD;
      markers.lifetime = object_viz_time;
      markers.header.stamp = rclcpp::Clock().now();
      ...
      if (pOSO->dtct || pOSO->InLane){
        ...
        if ((objInfo.find("Y") != std::string::npos)||(objInfo.find("y") != std::string::npos)) {
          /* yellow */
          Log::printLog("  -> Yellow Cone");
          markers.color.r = 1.0;
          markers.color.g = 1.0;
          markers.color.b = 0.0;
        } else if (!strcmp(pObj->Cfg.Info, "Left Blue Cone")) {
          /* blue */
          Log::printLog("  -> Blue Cone");
          markers.color.r = 0.0;
          markers.color.g = 0.3;
          markers.color.b = 1.0;
        } else {
          /* orange start cones */
          Log::printLog("  -> Orange Cone");
          markers.color.r = 1.0;
          markers.color.g = 0.6;
          markers.color.b = 0.0;
        }
...
int CMNodeHelloCM::userTestrunStartAtBegin() {
  ...
  if (VehicleInfo_Err == 0) {
    ...
    { /* set up odometry publisher + job */
      typedef CMJob::RosPublisher<nav_msgs::msg::Odometry> Odometry;
      Log::printLog("  -> Initializing odometry publisher");
      auto job = std::make_shared<Odometry>(nhp_, "Odometry");
      job->setCycleTime(100.0);
      job->registerCallback(&CMNodeHelloCM::odometryFillMsg, this);
      scheduler_.addJob(job);
    }
    ...
    { /* set up ObjectList publisher + job */
      ...
      if (idxP != -1) {
        ...
        typedef CMJob::RosPublisher<asurt_msgs::msg::LandmarkArray> FullLandmarkList;
        auto landjob = std::make_shared<FullLandmarkList>(nhp_, "/Landmarks/Full");
        landjob->setCycleTime(100.0);
        landjob->setCycleOffset(CMNode.Sensor.Object.nCycleOffset);
        landjob->registerCallback(&CMNodeHelloCM::FullLandmarkListFillMsg, this);
        scheduler_.addJob(landjob);

        typedef CMJob::RosPublisher<asurt_msgs::msg::LandmarkArray> LandmarkList;
        auto lj = std::make_shared<LandmarkList>(nhp_, "/Landmarks/Observed");
        lj->setCycleTime(100.0);
        lj->setCycleOffset(CMNode.Sensor.Object.nCycleOffset);
        lj->registerCallback(&CMNodeHelloCM::landmarklistFillMsg, this);
        scheduler_.addJob(lj);
      } else {
