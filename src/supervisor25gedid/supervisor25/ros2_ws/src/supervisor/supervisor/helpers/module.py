...
class Module(Node):  # Module should inherit from Node properly
    ...
    def update_heartbeat_rate(self) -> None:
        ...
        try:
            ...
            """
            # 🔴 Force the node to become UNRESPONSIVE for testing (REMOVE this after testing)
            self.get_logger().info(f"Heartbeat count: {new_heartbeat_rate}")
            if new_heartbeat_rate < 0:  # Lower threshold for quicker testing
    ...
    def launch(self) -> None:
        ...
        # Start the ROS 2 launch process in a separate process
            ...
            self.moduleHandle = self.launch_service
            self.state = "RUNNING"
        except Exception as e:
            self.get_logger().error(f"Failed to launch module: {e}")
